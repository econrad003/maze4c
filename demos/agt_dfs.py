"""
demos.agt_dfs - create a depth-first search maze using the arc growing tree
    module (mazes.Algorithms.growing_tree2 via mazes.AGT.dfs)
Eric Conrad
Copyright ©2024 by Eric Conrad.  Licensed under GPL.v3.

DESCRIPTION

    This is depth-first search (or DFS, or recursive bactracker implemented
    iteratively using a stack) with an arc-based architecture instead of a
    vertex-based architecture.
...
def make_maze(maze:"Maze", start_cell:"Cell", shuffle:bool=True) -> "Status":
    """carve the maze"""
    return dfs(maze, start_cell=start_cell, shuffle=shuffle)
...
def parse_args(argv):
    ...
    DESC = "Depth-first search (DFS) arc-growing tree demonstration"
    EPI = ""
    default_dim = (8, 13)

...
def main(grid_args:tuple, graphics_args:tuple, alg_args):
    ...
    if alg == None:
        ...
        if start_cell:
            ...
            # display the result
    monochrome, title = graphics_args
    fills = {}
    if not monochrome:
        for cell in grid:
...
def parse_args(argv):
    ...
    graphics.add_argument('-M', '--monochrome', action="store_true", \
        help=f'set this option for a plain black on white graph.  If this' \
        + ' option is not set, the cell colors will be determined' \
        + ' automatically.')
    ...
    graphics_args = (args.monochrome, title)
    ...
    main(grid_args, graphics_args, alg_args)

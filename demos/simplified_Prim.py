"""
demos.simplified_Prim - create a maze using simplified "Prim"
Eric Conrad
Copyright ©2024 by Eric Conrad.  Licensed under GPL.v3.

DESCRIPTION

    Here we use the simplified Prim algorithm to create a maze.
...
def make_not_Prim(maze, shuffle:bool=True) -> NotPrim.Status:
    """carve the maze"""
    return NotPrim.on(maze, shuffle=shuffle)

def main(rows:int, cols:int, color, output=None,
         tree_args:dict={}, console:bool=False, gui:bool=True):
    """the main entry point"""
    maze = make_grid(rows, cols)
    print(make_not_Prim(maze, **tree_args))
...
def parse_args(argv):
    ...
    DESC = "Simplified 'Prim' demonstration"
    EPI = ""
    default_dim = (8, 13)

"""
demos.vgt_vprim - create a "Vertex Prim" maze using the vertex growing tree
    module (mazes.Algorithms.growing_tree1 via mazes.VGT.vprim)
Eric Conrad
Copyright ©2024 by Eric Conrad.  Licensed under GPL.v3.

DESCRIPTION

    This is "Vertex Prim".  It is implemented using a priority queue.
    We let the priority queue class assign priorities to vertices as they
    are first encountered using the cache feature implemented in the class.
...
def make_maze(maze:"Maze", start_cell:"Cell",
    ...
    """carve the maze"""
    return vprim(maze, start_cell=start_cell, shuffle=shuffle, action=action)

def main(dim, start, shuffle, action, color, output, console, gui):
...
def parse_args(argv):
    ...
    DESC = 'Vertex "Prim" growing tree demonstration'
    EPI = 'The similarities between vertex "Prim" and Prim\'s algorithm' \
    ...
    maze.add_argument('-a', '--action', type=str, default='unstable', \
        help='this can be set to "stable", "antistable" or "unstable.' \
        + ' (Only the first character matter; upper case is accepted.'
        + ' Values other than "s", "a" or "u" raise ValueError. The' \
        + ' default is "unstable".)')
    ...
    main(args.dim, args.start, shuffle, args.action, args.pen, args.output,

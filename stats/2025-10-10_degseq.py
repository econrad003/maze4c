...
nokwargs = dict()
    ...
    # 36 x 36 = 4 passes of Fractal Tess from single cell with
    # alternating 2 x 3 and 3 x 2 tilings
    # 1296 cells, 1295 passages
grids = OblongGrid, (36, 36), nokwargs, 4
...
class FracTess(object):
    ...
    def on(cls, symmetries:bool, Group:callable=DihedralGroup):
        """create a 16x16 fractal maze"""
            # tiling: 2x3 or 3x2, alternating, 4 passes
        tess1 = FractalTessellation(3, 2, symmetries)
        tess2 = FractalTessellation(2, 3, symmetries)
        maze = tess1.on(passes=1, SymmetryGroup=Group, verbose=False)
        maze = tess2.on(maze, passes=1, SymmetryGroup=Group, verbose=False)
        maze = tess1.on(maze, passes=1, SymmetryGroup=Group, verbose=False)
        return tess2.on(maze, passes=1, SymmetryGroup=Group, verbose=False)

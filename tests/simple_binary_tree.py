...
def testEN(rows:int, cols:int):
    """create an east/north simple binary tree"""
    print("  east/north binary tree")
    ...
    for cell in maze.grid:
        ...
        if j == cols - 1:           # last column
            ...
            if i < rows - 1:        # all but top in column
                assert cell.is_linked(cell.north)
                assert cell.north.is_linked(cell)
        assert len(list(cell.passages)) <= 3        # binary tree
        assert len(list(cell.passages)) >= 1

    print("  east/north pass...")

def test1(rows:int, cols:int, onward:str, upward:str):
    """create a simple binary tree"""
    print(f"  {onward}/{upward} binary tree")
    ...
    maze = Maze(OblongGrid(rows, cols))
    maze.grid.set_format("leader", "    ")
    status = BinaryTree.on(maze, onward=onward, upward=upward)
    print(maze)
    print(status)
    assert status.fetch_item("visits") == len(maze.grid) + 1
    assert status.fetch_item("cells") == len(maze.grid)
    ...
    for cell in maze.grid:
        ...
        if i == toprow:             # top row
            ...
            if j != lastcol:        # all but last in row
        ...
        if j == lastcol:            # last column
            ...
            if i != toprow:         # all but top in column
...
def test2(rows:int, cols:int, bias:float, warnlow:float, warnhigh:float):
    """create an east/north simple binary tree with a bias"""
    print(f"  east/north binary tree with bias p={bias}")
    maze = Maze(OblongGrid(rows, cols))
    maze.grid.set_format("leader", "    ")
    status = BinaryTree.on(maze, bias=bias)
    print(maze)
    print(status)
    assert status.fetch_item("visits") == len(maze.grid) + 1
    assert status.fetch_item("cells") == len(maze.grid)
...
def weird(cell, m:float=0.5, **kwargs):
    """a strangely psychic coin"""
    i, j = cell.index
    if i < m*j-1:
        return False
    if i > m*j+1:
...
def test3(rows:int, cols:int):
    """create an east/north simple binary tree with a wierd bias"""
    print("  east/north binary tree with wierd bias")
    maze = Maze(OblongGrid(rows, cols))
    maze.grid.set_format("leader", "    ")
    status = BinaryTree.on(maze, flip=weird)
    print(maze)
    status.store_item("flip", "weird(cell)")
    print(status)
    assert status.fetch_item("visits") == len(maze.grid) + 1
    assert status.fetch_item("cells") == len(maze.grid)
    assert status.fetch_item("passages") == len(maze.grid) - 1
    ...
    print("  east/north pass with weird psychic coin...")
...
def tests(rows:int, cols:int):
    ...
    print("        *** failure of the bias tests might not indicate a problem ***")
    print("        *** Try running these tests more than once or with a       ***")
    print("        *** larger maze.                                           ***")
    test2(rows, cols, 0.25, 0.1, 0.4)
    test2(rows, cols, 0.5, 0.35, 0.65)
    test2(rows, cols, 0.75, 0.6, 0.9)
    print("        *** A cell-dependent bias                                  ***")
    test3(rows, cols)
...
def main(argv):
    ...
    DESC = "test the simple binary tree algorithm"
